...
def Score():
    with connection.cursor() as cursor:
        cursor.execute(
            "CREATE TABLE Score_snake (user VARCHAR(64) NOT NULL PRIMARY KEY, user_score BIGINT NOT NULL);"
        )
        print("table created")

def insert_user_name_from_console():
    ...
    try:
        ...
        with connection.cursor() as cursor:
            cursor.execute("""
                INSERT INTO Score_snake (user_name)
                VALUES (%s)
            """, (user_name))
            
        print("Name inserted from console")

    except Exception as ex:
        print("[INFO] Error while inserting name from console:", ex)
